...
| `ezvm` CLI | **`riftvm`** | Host CLI；过渡期保留 `ezvm` 兼容入口 |
...
- `.ezvm` 永久保持可打开、可导入；不强制批量改扩展名。
- 第一次启动发现旧目录时进行可回滚迁移，并写入完成标记。
- Keychain、Agent enrollment 与 manifest 先读新标识，未找到再读旧标识；成功读取后复制到新标识，不提前删除旧值。
- 旧 VM 路径保持有效；只有新建 VM 默认进入 RiftVM 目录。
- 所有迁移必须幂等，应用崩溃或用户中断后可安全重试。
...
如果需要完全无感保留 TCC 权限，可以保留旧 Bundle ID；但“全面改名”与未来独立品牌更适合采用新 Bundle ID，并接受一次明确的重新授权。这一决策必须在实施第一阶段锁定，之后不可反复切换。

### 3.5 Host ↔ Guest 协议

协议兼容优先级高于内部命名美观。

- 新 Host 必须能连接已安装 `ezvm-agent` 的旧 Guest。
- 新 `rift-agent` 必须能被旧版本 Host 拒绝或协商，而不是导致 Guest 不可启动。
- 保持现有 vsock 端口、消息结构、认证身份和 capability 名称兼容至少两个公开版本。
- 协议新增 `product`、`agentName`、`protocolVersion` 与 capabilities；品牌名不作为认证依据。
- systemd/OpenRC 服务采用 alias/迁移单元：旧 `ezvm-agent.service` 可启动新 binary，过渡后再反向别名。
- `/run/ezvm-agent`、`/mnt/ezvm-shared` 与 VirtioFS tag 第一阶段不变；新路径只能在双方能力协商后启用。
- 日志和 UI 可以立刻显示 Rift Agent，底层 legacy path 不需要在同一版本强制清除。
...
1. 新变量优先。
2. 未设置时读取旧 `EZVM_*`。
3. 使用旧变量时输出一次 deprecated warning。
4. 至少经过两个公开版本后，才讨论删除旧变量。

测试、fixture 与 `/tmp/ezvm-*` 前缀可以逐步改为 `/tmp/riftvm-*`，但不属于用户数据迁移。
...
- 建立一份真实旧版本测试 VM，用于整个迁移周期回归。

验收：能从基线 tag 构建并运行两个 App，CI 全绿，旧 Omarchy Guest 可启动。

### Phase 1 — 先加兼容层，不改用户可见名称

- 集中定义 `ProductIdentity`、路径、manifest kind、CLI 与环境变量别名。
- 增加旧路径/新路径双读与迁移测试。
- 增加 `.ezvm` 与 `.riftvm` 双格式测试。
- 增加旧 Host/new Agent、new Host/旧 Agent 的协议矩阵测试。
- 所有 hard-coded `EZVM` 按 brand、API、persistence、wire 四类标注。

验收：行为与当前版本完全一致，但所有危险标识已有兼容入口和测试保护。
...
### Phase 3 — macOS 产品身份与数据迁移

- App 名称改为 RiftVM / RiftVM Omarchy。
- Bundle ID、entitlements、日志 subsystem、Notification identifiers 更新。
- 实现 Application Support、UserDefaults、Keychain、VM root 和 enrollment 的幂等迁移。
- 新建 VM 使用 `.riftvm`，打开面板同时接受 `.ezvm`。
- 增加“RiftVM was formerly EZVM”一次性升级说明。
- 验证旧 VM、snapshot、saved state、thumbnail、USB/network 配置和 Guest credentials。

验收：在安装过当前正式版的 Mac 上升级，所有 VM 可见且可启动；失败可回滚；没有复制大磁盘。

### Phase 4 — Rift Agent 品牌与兼容发布
...
- 旧 service alias、旧 socket/path 与 capability aliases 保留。
- 增加 Agent 自升级/重启时的断线恢复与 key-up 清理。
- 文档统一为 Rift Agent，但诊断页明确显示 legacy compatibility 状态。

验收：四组 Host/Agent 新旧组合都有明确成功或受控降级结果，不能出现沉默失联。

### Phase 5 — CLI、制品、CI 与发行链

- 发布 `riftvm` CLI；App bundle 内保留 `ezvm` shim，提示迁移。
- Homebrew cask 新建 `riftvm`；旧 `ezvm` cask 标记迁移/冲突关系。
...
- 发布迁移公告，清楚说明新名字、数据兼容、权限重授予和旧 CLI 兼容期。
...
- 不把改名与输入策略重设计混在同一个提交。
- 不删除 `.ezvm` 支持，不批量复制或重写用户的大磁盘。
- 不默认共享整个 Home，不默认开放 Guest 服务端口。
- 不给 Guest 任意执行 Host shell 的接口。
- 不为追求“内部看起来干净”而破坏 wire protocol、systemd path 或旧 Agent。
- 不在没有真实旧版本 VM 的情况下宣称迁移成功。
...
- 当前 EZVM → RiftVM。
- 当前 EZVM Omarchy → RiftVM Omarchy。
- 旧 `.ezvm` 位于默认目录、自定义目录和外置磁盘。
- 旧 Keychain credential、enrollment、saved state 与 snapshots。
- 迁移中断后重启。
- RiftVM 与旧 EZVM 同时存在时给出明确处理，不争抢同一运行锁。

### Host/Guest 兼容矩阵

| Host | Guest Agent | 预期 |
|---|---|---|
| EZVM | ezvm-agent | 完整旧行为 |
| RiftVM | ezvm-agent | 兼容运行并提示可升级 |
| EZVM | rift-agent | 受控兼容或明确版本错误 |
| RiftVM | rift-agent | 完整能力 |
...
1. `Add RiftVM compatibility identity layer`
2. `Rename shared Swift modules and projects to RiftVM`
3. `Migrate macOS product identity and user data`
4. `Introduce Rift Agent with legacy aliases`
5. `Rename CLI, CI and release artifacts`
6. `Launch riftvm.com and publish migration guide`
...
- 数据迁移第一版只复制小型 metadata；大 VM bundle 原地引用，不搬迁。
- 新记录写入失败时继续读旧记录，不删除旧 Keychain/Agent enrollment。
- Website/DNS 可立即回滚到上一版本；旧 URL 保留重定向。
- 新 Agent Overlay 发布失败时，factory image 仍可使用最后一个已验证旧 Agent。
- 每个发布制品附 source revision、tree state、协议版本和迁移 schema version。
...
这样可以做到真正的全面改名，同时不牺牲已经发布的 VM、Guest Agent、权限和发行可靠性。
